...
def successors(state):
    ...
    free = set([i for i in range(N ** 2)])
    cars_data = {}
    for (c, positions) in state:
        if c not in '@|':
            direction = 'h' if abs(positions[0] - positions[1]) == 1 else 'v'
            min_pos, max_pos = min(positions), max(positions)
            cars_data[c] = (direction, min_pos, max_pos)
        if c != '@':
            free.difference_update(set(positions))

    for (c, positions) in state:
        if c not in '@|':
            (direction, min_pos, max_pos) = cars_data[c]
            step = 1 if direction == 'h' else N

            i = 1
            while True:
                left_top_pos = min_pos - step * i
                if left_top_pos in free:
                    new_state = dict(state)
                    new_state[c] = tuple(p - step * i for p in positions)
                    action = (c, -step * i)
                    results[tuple(new_state.items())] = action
                else:
                    break
                i += 1
            ...
            i = 1
            while True:
                down_right_pos = max_pos + step * i
                if down_right_pos in free:
                    new_state = dict(state)
                    new_state[c] = tuple(p + step * i for p in positions)
                    action = (c, step * i)
                    results[tuple(new_state.items())] = action
...
def grid(cars, N=N):
    ...
    goal = (N * N // 2 - 1,)
    walls = tuple(w for w in walls if w not in goal)
    return cars + (('|', walls), ('@', goal))
...
p = solve_parking_puzzle(puzzle1, N=8)
