...
def double_out(total):
    ...
    ordered_points = [0] + sorted({i * j for i in range(1, 21) for j in [1, 2, 3]}.union({25, 50}),
                                  reverse=True)
    for dart1 in ordered_points:
        for dart2 in ordered_points:
            dart3 = total - dart1 - dart2
            if dart3 in ordered_points:
                darts = [d for d in [dart1, dart2, dart3] if d != 0]
                names = list(map(name, darts))
                if names and names[-1].startswith('D'):
                    return names
...
If you aim for a thick single ring, it is about 5 times thicker than the 
thin rings, so your miss ratio is reduced to 1/5th, and of these, half 
go to the double ring and half to the triple.  So with miss=0.1, 0.01 
will go to each of the double and triple ring.  
Finally, for the bulls-eyes. If you aim for the single bull, 1/4 of 
your misses go to the double bull and 3/4 to the single ring.  
