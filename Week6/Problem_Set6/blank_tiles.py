...
POINTS = dict(A=1, B=3, C=3, D=2, E=1, F=4, G=2, H=4, I=1, J=8, K=5, L=1, M=3, N=1, O=1, P=3, Q=10,
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if PRE in PREFIXES:
        ...
        if is_letter(sq):
            ...
            if '_' in hand:     # allow blanks
                for C in possibilities:
                    c = C.lower()
                    add_suffixes(hand.replace('_', '', 1), pre + c, start, row, results)
    return results
...
def best_play(hand, board):
    ...
    return max(plays) if plays else NO_PLAY
